...
async function startFileSearchServices() {
  ...
  /*********************************************************************/
  /*
   * Adaptation of REAL TIME FILE SEARCH by Osvaldas
   * https://osvaldas.info/real-time-search-in-javascript#image-1
   */
  /*********************************************************************/
  (function(document, window, index) {
    ...
    (function(document, window, index) {
      ...
      [].forEach.call(items, function(entry) {
        itemsIndexed.push(
          entry.textContent.replace(/\s{2,}/g, " ").toLowerCase()
        );
      });
    ...
    (function(document, window, index) {
      ...
      input.addEventListener("keyup", function(e) {
        item = [].filter.call(items, function(entry) {
          return !hasElementClass(entry, "is-hidden");
        });
